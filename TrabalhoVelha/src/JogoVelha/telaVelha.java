...
public class telaVelha extends JFrame {
			...
			if((j.getMousePosition(false)!=null)&&!t1.verificarGanhador() && !t1.empate()) {
				...
				if(t1.verificarPosicao(linha, coluna)) {
					...
					if(!t1.verificarGanhador() && !t1.empate()) {
						...
						jbStatus.setText("Alguem ganhou");
						if (t1.getVerf() == 'X') {
	...
	private void desfazer() {
		if((!tab.isEmpty())&&(!t1.verificarGanhador())) {
			int co = t.pop();
			int li = t.pop();
		    t1.jogar(li, co, '-');
		    String n = "";
	...
	private void initComponents() {
		...
		this.contentPane.setLayout(new MigLayout("", "[60.00][80.00,fill][80.00,fill][80.00,fill][60.00]", "[30.00][fill][80.00,fill][80.00,fill][80.00,fill][40.00,fill]"));
